    ...
    while(1){
        ...
        if (Status != XST_SUCCESS){
			...
			if(RxBuffer != 0xF0)
        	{
        		...
        		if(RxBuffer == 0x1D) //iduca boja
        		{
        			if(boja==0x0000FF00) boja = 0x00FF0000;
        			else if(boja==0x00FF0000) boja = 0x00FFFF00;
        			else if(boja==0x00FFFF00) boja = 0x000000FF;
        			else if(boja==0x000000FF) boja = 0x0000FF00;
        		}
        		if(RxBuffer == 0x1B) //prethodna boja
        		{
					if(boja==0x0000FF00) boja = 0x000000FF;
					else if(boja==0x00FF0000) boja = 0x0000FF00;
					else if(boja==0x00FFFF00) boja = 0x00FF0000;
					else if(boja==0x000000FF) boja = 0x00FFFF00;
				}
        		if(RxBuffer == 0x16) brLinija=1;
        		if(RxBuffer == 0x1E) brLinija=2;
				if(RxBuffer == 0x26) brLinija=3;
				if(RxBuffer == 0x25) brLinija=4;

				XTft_ClearScreen(&TftInstance);
				for(i=100;i<341;i++)
				{
					for(j=20;j<(21*brLinija);j+=20)
					{
						XTft_SetPixel(&TftInstance,i,j,boja);
					}
				}
			}
        }
